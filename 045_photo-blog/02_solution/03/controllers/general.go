...
func(ctl Controller) Index(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	c := getCookie(w, req)
	xs := strings.Split(c.Value, "|")
	ctl.tpl.ExecuteTemplate(w, "index.gohtml", xs[1:]) //only send over images
}

func(ctl Controller) IndexSubmission(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	c := getCookie(w, req)
